abstract class StudentPrototype
{
    public string _name;
    public int _password;
    private Random random = new();
    public StudentPrototype(string name, int password)
    {
        _name = name;
        _password = password;
    }
    ...
    private double AnswerQuestion(Tutor tutor, int testIndex, int questionIndex)
    {
...
class ConcreteStudent : StudentPrototype
{
    public ConcreteStudent(string name, int password) : base(name, password) { }
    public override StudentPrototype Clone()
    {
        return new ConcreteStudent(_name, _password);
    }
}